// analyse.js – Multi-KI Analyse nur für ausgewähltes Asset
...
async function fetchHistoricalData(sym, days){
  try{
    if(sym.includes("USD")){
      ...
      const json = await res.json();
      return json.prices ? json.prices.map(p=>p[1]) : Array(days).fill(100);
    } else {
      ...
      const json = await res.json();
      return (json.s==="ok" && json.c?.length>0) ? json.c.map(Number) : Array(days).fill(await fetchQuote(sym));
    }
...
async function run(){
  ...
  const allPrognosen = [];
  for(let i=0;i<4;i++){allPrognosen[i]=await predictKI(hist,period,i);}
  ...
  statusDiv.textContent="Fertig"; loaderDiv.textContent="–";
}
